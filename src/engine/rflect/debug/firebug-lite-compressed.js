(function(_scope) {
  ...
  var pi = _scope.pi;
  pi.version = 1.0;
  pi.env = {ie:/MSIE/i.test(navigator.userAgent),ie6:/MSIE 6/i.test(navigator.userAgent),ie7:/MSIE 7/i.test(navigator.userAgent),ie8:/MSIE 8/i.test(navigator.userAgent),firefox:/Firefox/i.test(navigator.userAgent),opera:/Opera/i.test(navigator.userAgent),webkit:/Webkit/i.test(navigator.userAgent)};
  pi.util = {IsArray:function(_object) {
    ...
    return{height:pi.env.ie ? Math.max(document.documentElement.clientHeight, document.body.clientHeight) : window.innerHeight,width:pi.env.ie ? Math.max(document.documentElement.clientWidth, document.body.clientWidth) : window.innerWidth}
    ...
    if (!pi.util.Element.hasClass(_element, _class))pi.util.Element.setClass(_element, pi.util.Element.getClass(_element) + " " + _class)
  ...
  pi.element = pi.base.extend({"$Constructor":function(_tag) {
    ...
    ["TEXTAREA","INPUT"].indexOf(this.environment.getName()) > -1 ? (this.environment.getElement().value = _value) : (this.environment.getElement().innerHTML = _value);
    return this
    ...
    return this._parent_.query("{0}{1}".format((_tag ? "{0}".format(_tag) : "/*"), _attributeName || _attributeValue ? "[contains(concat(' ', @{0}, ' '),' {1} ')]".format(_attributeName || "", _attributeValue || "") : ""))
  ...
  pi.xhr.body = {"abort":function() {
    ...
    for (var key in this.environment.getHeader())this.environment.getApi().setRequestHeader(key, this.environment.getHeader()[key]);
    this.environment.getApi().send(this.environment.getType() == "GET" ? "" : dataUrl)
  }};
  pi.xhr.body.environment = {"_async":true,"_api":null,"_cache":true,"_callback":[],"_channel":null,"_data":{},"_header":{},"_mimeType":null,"_multipart":false,"_type":"GET","_timeout":0,"_url":"","addCallback":function(_options, _fn) {
  ...
  pi.xpath = function(_expression, _resultType, _contextNode, _namespaceResolver, _result) {
    var contextNode = _contextNode || document,expression = _expression || "",namespaceResolver = _namespaceResolver || null,result = _result || null,resultType = _resultType || "ANY_TYPE";
  ...
  Number.prototype.range = function(_pattern) {
    for (var value = String(this),isFloat = /\./i.test(value),i = isFloat.toggle(parseInt(value.split(".")[0]), 0),end = parseInt(value.split(".")[isFloat.toggle(1, 0)]),array = []; i < end; i++) {
  ...
  String.prototype.unicode = function() {
    ...
    for (var i = obj.length - 1; i >= 0; i--)str = "\\u{0}{1}".format(String(obj[i].charCodeAt(0).base(16)).leftpad(4, "0"), str);
    return str
  };
...
var firebug = {env:{"cache":{},"ctmp":[],"dIndex":"console","init":false,"ml":false,"objCn":[]},init:function() {
  ...
  with (firebug) {
    ...
    el.left.console.monitor = new pi.element("DIV").insert(new pi.element("DIV").attribute.addClass("Monitor").insert(el.left.console.container));
    el.left.console.container.child.add(new pi.element("DIV").attribute.addClass("InputArrow").update(">>>"));
    el.left.console.input = new pi.element("INPUT").attribute.set("type", "text").attribute.addClass("Input").event.addListener("keydown", listen.consoleTextbox).insert(new pi.element("DIV").attribute.addClass("InputContainer").insert(el.left.console.container));
    el.right.console = {};
    ...
    if (!pi.env.ie6)el.right.html.nav.dom = new pi.element("A").attribute.addClass("Tab").event.addListener("click", d.html.navigate.curry(firebug, "dom")).update("DOM").insert(el.right.html.nav.container);
  ...
  with (firebug) {
    ...
    for (var key in obj) {
      try {
        ...
        var link = new pi.element("A").attribute.addClass(typeof value == "object" && Boolean(value) ? "Property Object" : "Property").update(property).insert(left);
  ...
  with (firebug) {
    var element = _element || document.documentElement,parent = _parent || el.left.html.container,returnParentEl = _returnParentElementByElement || null,returnParentVal = null;
  ...
  ;
  with (firebug) {
    el[_bgInspector ? "bgInspector" : "borderInspector"].environment.addStyle({"width":_element.offsetWidth + "px","height":_element.offsetHeight + "px","top":top - (_bgInspector ? 0 : 2) + "px","left":left - (_bgInspector ? 0 : 2) + "px","display":"block"});
    if (!_bgInspector) {
  ...
  with (firebug) {
    ...
    if (!d.scripts.lineNumbers)el.left.scripts.container.child.add(new pi.element("DIV").attribute.addClass("CodeContainer").update(source)); else {
      ...
      for (var i = 0; i < source.length; i++) {
        el.left.scripts.container.child.add(new pi.element("DIV").child.add(new pi.element("DIV").attribute.addClass("LineNumber").update(i + 1), new pi.element("DIV").attribute.addClass("Code").update("&nbsp;" + source[i]), new pi.element("DIV").environment.addStyle({"clear":"both"})))
      }
    }
  }
  ...
  with (firebug) {
    ...
    for (var i = 0; i < collection.length; i++) {
      ...
      el.button.scripts.selectbox.child.add(new pi.element("OPTION").attribute.set("value", i).update(item.src || item.baseURI || ".."))
    }
    d.scripts.open(d.scripts.index)
  }
  ...
  with (firebug) {
    ...
    for (var i = 0; i < d.xhr.objects.length; i++) {
      ...
      el.left.xhr.responseContent.child.add(new pi.element("span").child.add(new pi.element("A").event.addListener("click", d.str.open.curry(window, response)).update("&nbsp;" + response.substring(0, 50))))
    }
    ;
  ...
  with (firebug) {
    ...
    switch (open) {case"console":d.navigateRightColumn(_index);break;case"html":d.navigateRightColumn(_index, true);d.html.openHtmlTree();break;case"css":d.navigateRightColumn(_index, true);d.css.refresh();break;case"scripts":d.navigateRightColumn(_index);d.scripts.refresh();break;case"dom":d.navigateRightColumn(_index);if (el.left.dom.container.environment.getElement().innerHTML == "")d.dom.open(eval(el.button.dom.textbox.environment.getElement().value), el.left.dom.container);break;case"xhr":d.navigateRightColumn(_index);d.xhr.open();break}
  }
},refreshSize:function() {
  with (firebug) {
    el.main.environment.addStyle({"width":pi.util.GetWindowSize().width + "px"});
    if (pi.env.ie6)el.main.environment.addStyle({"top":pi.util.GetWindowSize().height - el.main.environment.getSize().offsetHeight + "px"})
  }
  ...
  with (firebug) {
    ...
    el.left.console.input.update(d.console.history.length > d.console.historyIndex ? d.console.history[d.console.historyIndex] : "")
  }
  ...
  with (firebug) {
    ...
    if (d.inspector.enabled && target != document.body && target != document.firstChild && target != document.childNodes[1] && target != el.borderInspector.environment.getElement() && target != el.main.environment.getElement() && target.offsetParent != el.main.environment.getElement())d.inspector.inspect(target)
  }
  ...
  with (firebug) {
    ...
    for (var i = 0; i < source.length; i++) {
      var item = source[i] + "}",rule = !pi.env.ie ? item : item.split(/{|}/),collection = document.getElementsByTagName("style"),style = collection.length > 0 ? collection[0] : document.body.appendChild(document.createElement("style"));
